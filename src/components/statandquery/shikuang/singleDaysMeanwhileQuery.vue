    ...
    <div class="heightMax">
      <div class="station">
        <Breadcrumb class="ml32">
          ...
          <Breadcrumb-item class="blue">单站多日同时</Breadcrumb-item>
        </Breadcrumb>
...
<script>
import { Daywhilethead } from 'common/js/table'
import { GetDaywhildQuery, GettownList } from 'api/stationlist'
...
export default {
  ...
  methods: {
    ...
    // 列表查询
    _GetDaywhildQuery (page, search) {
      this.loading = true
      GetDaywhildQuery(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
    ...
    // 页码
    changePage (page) {
      this._GetDaywhildQuery(page)
    }
  },
  mounted() {
    this._GettownList()
    this._GetDaywhildQuery()
  }
}
</script>
